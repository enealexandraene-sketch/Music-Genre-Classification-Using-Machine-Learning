...
# Train Perceptron model on scaled data
perceptron = Perceptron(max_iter=100)
perceptron.fit(X_train_scaled, y_train)

# Predict and evaluate
y_pred_perc = perceptron.predict(X_test_scaled)
accuracy_perc = balanced_accuracy_score(y_test, y_pred_perc)
print(f"Perceptron Balanced Accuracy: {accuracy_perc:.4f}")
...
# In binary case, convert decision function output properly
if len(np.unique(y)) == 2 and len(y_score.shape) == 1:
    y_score = np.vstack([-y_score, y_score]).T

# Compute ROC AUC score
roc_auc_macro = roc_auc_score(y_test_bin, y_score, average="macro", multi_class="ovr")
print(f"Perceptron ROC AUC: {roc_auc_macro:.4f}")
...
plt.plot(all_fpr, mean_tpr, color="darkorange", label=f"Perceptron AUC = {roc_auc_macro:.2f}", lw=2)
...
#f1 score
f1_perc = f1_score(y_test, y_pred_perc, average="macro") 
print(f"Perceptron F1 Score: {f1_perc:.4f}")

# precision score
precision_perc = precision_score(y_test, y_pred_perc, average="macro", zero_division=0)
print(f"Perceptron Precision: {precision_perc:.4f}")

# recall score
recall_perc = recall_score(y_test, y_pred_perc, average="macro", zero_division=0)
print(f"Perceptron Recall: {recall_perc:.4f}")