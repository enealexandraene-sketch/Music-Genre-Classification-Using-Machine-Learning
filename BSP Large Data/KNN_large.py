...
# select numerical columns without encoded genre
...
# prepare to assign colors for each genre using Seaborn's palette
...
for genre_name in unique_genres:
    ...
    ax.scatter(X_embedded[mask, 0], X_embedded[mask, 1],
               color=genre_to_color[genre_name],
               s=20, alpha=0.7, label=genre_name)

# Move legend outside the plot
ax.legend(bbox_to_anchor=(1.02, 1), loc="upper left", title="Genres", borderaxespad=0.)
...
# precision score
precision_knn = precision_score(y_test, y_knn_pred, average="macro", zero_division=0)
print(f"KNN Precision: {precision_knn:.4f}")

# recall score
recall_knn = recall_score(y_test, y_knn_pred, average="macro", zero_division=0)
